...
type_vis = st.selectbox('Select the type of viualization you would like to see:', options=vis_to_use)
...
if type_vis == 'scatterplot':
    ...
    if answer and answer2:
        try:
            st.plotly_chart(px.bar(df, x=answer, y=answer2, hover_data = ['name']), use_container_width=True)
        except BaseException:
            print("Error visualizing those combination of columns")
